...
function transformCursor(cursor, delta, isOwnOp) {
  ...
  console.log(cursor, delta, delta.transformPosition(cursor, !isOwnOp));
...
function transformPresence(presence, operation, isOwnOperation) {
  const ops = operation.o ? [operation.o] : operation;
  console.log(presence,operation)
  var selections = presence.s;

  for (var i = 0, l = selections.length; i < l; ++i) {
    var selection = selections[i];
    var newStart = transformCursor(selection[0], op, isOwnOperation);
    var newEnd =
      selection[0] === selection[1]
        ? newStart
        : transformCursor(selection[1], op.o, isOwnOperation);
    selections[i] = [[newStart, newEnd]];
  }
...
function comparePresence(presence1, presence2) {
  console.log(presence1, presence2);
